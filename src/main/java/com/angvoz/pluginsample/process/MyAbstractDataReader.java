 ...
 * @author TODO
 */
public abstract class MyAbstractDataReader {
  private static final Logger LOG = Logger.getInstance(MyAbstractDataReader.class);
  ...
  public MyAbstractDataReader(SleepingPolicy sleepingPolicy) {
    ...
    if (myFinishedFuture == null) {
      myFinishedFuture = executeOnPooledThread(() -> {
        if (StringUtilRt.isEmptyOrSpaces(presentableName)) {
          ...
          ConcurrencyUtil.runUnderThreadName("MyAbstractDataReader: " + presentableName, this::doRun);
        }
      });
    }
  }
   ...
   * <p>
   * If the execution handler assumes that reader handles the blocking mode, while it doesn't, it will result into premature stream close.
   *
  ...
  /**
   * Non-blocking read returns the control back to the execution handler when there is no data to read.
   *
  ...
  /**
   * <h2>Blocking</h2>
   * In Java you can only read data from child execution's stdout/stderr using blocking {@link InputStream#read()}.
   * (Async approach like {@link java.nio.channels.SelectableChannel} is not supported for execution's streams,
   * although some native api may be used).
   * Thread stays blocked by {@link InputStream#read()} until some data arrived or stream is closed (because of execution death).
   * It may lead to issues like {@code IDEA-32376}: you can't unlock blocked thread (at least non-daemon) otherwise than by killing
   * execution (and you may want to keep it running). {@link Thread#interrupt()} doesn't work here.
   * This approach is good for short-living processes.
   * If you know for sure that execution will end soon (i.e. helper execution) you can enable this behaviour using {@link #BLOCKING} policy.
   ...
   * <li>Repeat</li>
   * </ol>
   * This "busy-wait" anti-pattern is the only way to exit thread leaving execution alive. It is required if you want to "disconnect" from
   * user execution and used by {@link #NON_BLOCKING} (aka non-blocking) policy. Drawback is that execution may finish (when {@link Process#waitFor()} returns)
   ...
   * For user execution that may run forever, even after idea is closed, and user should have ability to disconnect from it
    ...
    try {
      ...
      while (true) {
        ...
        try {
          ...
          if ("The execution cannot access the file because another execution has locked a portion of the file".equals(e.getMessage())) {
        ...
        if (!stopSignalled) {
          // if execution stopped, there is no sense to sleep, just check if there is unread output in the stream
          beforeSleeping(read);
